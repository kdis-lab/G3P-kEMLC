...
public class Main {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<iters; i++) {
			String ind1 = creator.create(nMax, maxDepth);
			String ind2 = creator.create(nMax, maxDepth);
			
			System.out.println(ind1);
			System.out.println(ind2);
			if(!utils.checkInd(ind1)) {
				System.out.println("ind not feasible: " + ind1);
				System.exit(1);
			}
			if(!utils.checkInd(ind2)) {
			...
			if(!utils.checkInd(crossed[0])) {
				System.out.println("crossed not feasible: " + crossed[0]);
				System.exit(1);
			}
			if(!utils.checkInd(crossed[1])) {
